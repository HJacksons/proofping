...
import { proofRequestVisibilityValues } from "@/lib/proof-requests/visibility";

export const createProofRequestSchema = z
  .object({
    title: z
      .string()
      .trim()
    ...
    visibility: z
      .enum(proofRequestVisibilityValues)
      .default("PRIVATE_LINK"),
  })
  .superRefine((request, context) => {
    if (request.visibility === "LOCAL_DISCOVERY" && !request.locationHint) {
      context.addIssue({
        code: "custom",
        path: ["locationHint"],
        message: "Add a city or neighborhood for nearby helpers.",
      });
    }
  });
